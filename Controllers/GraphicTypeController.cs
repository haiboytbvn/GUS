...
namespace GUSLibrary.Controllers
{
    ...
    public class GraphicTypeController : Controller
    {
        ...
        #region Constructor
        public GraphicTypeController(GUSLibraryDbContext context)
        {
            DbContext = context;
        }
        ...
        // GET: api/values
        [HttpPost("GetGraphicTypeList")]
        public IActionResult GetGraphicTypeList([FromBody]SearchGeneralFilterViewModel data)
        {
            ...
            var itemList = DbContext.GraphicTypes.Where(x =>
           ...
           .Where(z => z.IsLatest && !(z.IsDeleted) && z.IsActive).OrderBy(x => x.Name);
            ...
            // pass in the Model
            var result = new GraphicTypeListViewModel();
            result.Data = new List<GraphicTypeViewModel>();
            result.Paging = new Pagination();
            if (data.Paging.PageNumber == 0)
            {
                // if pagenumuber ==0 , get All
                result.Data = itemList.Select(x => TinyMapper.Map<GraphicTypeViewModel>(x)).ToList();
            ...
            result.Data = list.Select(x => TinyMapper.Map<GraphicTypeViewModel>(x)).ToList();
        ...
        // GET by Id
        [HttpGet("GetGraphicTypeById/{id}")]
        public IActionResult GetGraphicTypeById(Guid id)
        {
            Guid companyIdFromSearchData = Guid.Empty;
            Guid companyId = CompanyIdHelper.GetCompanyId(companyIdFromSearchData);
            var item = DbContext.GraphicTypes.Where(x => (companyId == Guid.Empty) || (x.CompanyId == companyId)).FirstOrDefault(i => i.Id == id);
            if (item != null) return new JsonResult(TinyMapper.Map<GraphicTypeViewModel>(item), DefaultJsonSettings);
            return NotFound(new { Error = $"ID {id} has not been found" });
        }


        // GET api/values/slug
        [HttpGet("GetGraphicTypeBySlug/{slug}")]
        public IActionResult GetGraphicTypeBySlug(string slug)
        {
            Guid companyIdFromSearchData = Guid.Empty;
            Guid companyId = CompanyIdHelper.GetCompanyId(companyIdFromSearchData);
            var item = DbContext.GraphicTypes.Where(x => (companyId == Guid.Empty) || (x.CompanyId == companyId)).Where(x=>x.IsLatest && !x.IsDeleted).FirstOrDefault(i => i.Slug == slug);
            if (item != null) return new JsonResult(TinyMapper.Map<GraphicTypeViewModel>(item), DefaultJsonSettings);
            return NotFound(new { Error = $"Slug {slug} has not been found" });
        }

        // Get item by code
        [HttpGet("GetGraphicTypeByCode/{code}")]
        public IActionResult GetGraphicTypeByCode(string code)
        {
        ...
        [HttpGet("GetGraphicTypeListByCode/{code}")]
        public IActionResult GetGraphicTypeListByCode(string code)
        {
            if (!string.IsNullOrEmpty(code))
            {
                Guid companyIdFromSearchData = Guid.Empty;
        ...
        // POST api/values
        [HttpPost("AddGraphicType")]
        public IActionResult Add([FromBody]GraphicTypeViewModel data)
        {
            if (data == null)
                return NotFound(new { error = "Invalid Data" });
            // return a HTTP Status 404 (Not Found) if we couldn't find a suitable item.
            ...
            string code = ch.GetTypeCode(EnumHelper.LibType.LibGraphicType.ToString());

            // Check if slug is duplicate or not
            string slug = SlugHelper.GetSlug(data.CompanyId, DbContext, data.Name, EnumHelper.LibType.LibGraphicType.ToString());
            var fabricstype = new LibGraphicType()
            {
                Id = Guid.NewGuid(),
            ...
            DbContext.GraphicTypes.Add(fabricstype);
        ...
        // PUT api/values/5
        [HttpPut("UpdateGraphicType")]
        [Authorize]
        public IActionResult Update([FromBody]GraphicTypeViewModel data)
        {
            if (data != null)
            {
                var fabricstype = DbContext.GraphicTypes.FirstOrDefault(i => i.Id.Equals(data.Id));
                ...
                if (fabricstype != null)
                {
                    ...
                    var fabtype = new LibGraphicType()
                    {
                        Id = Guid.NewGuid(),
                    ...
                    DbContext.GraphicTypes.Add(fabtype);
                    ...
                    DbContext.GraphicTypes.Update(fabricstype);
        ...
        // DELETE api/values/5
        [HttpDelete("DeleteGraphicType/{id}")]
        [Authorize]
        public IActionResult Delete(Guid id)
        {
            var item = DbContext.GraphicTypes.FirstOrDefault(i => i.Id.Equals(id));
            if (item != null)
            {
                item.IsDeleted = true;
                item.IsLatest = false;  
                DbContext.GraphicTypes.Update(item);
            }
            DbContext.SaveChanges();
        ...
        private List<GraphicTypeViewModel> ToItemViewModelList(IEnumerable<LibGraphicType> accessoryDesc)
        {
            var result = Enumerable.ToList(accessoryDesc.Select(TinyMapper.Map<GraphicTypeViewModel>));
            return result;
        }
