...
namespace GUSLibrary.Controllers
{
    ...
    public class FabricsTypeController : Controller
    {
        ...
        #region Constructor
        public FabricsTypeController(GUSLibraryDbContext context)
        {
            DbContext = context;
        }
        ...
        // GET: api/values
        [HttpPost("GetFabricsTypeList")]
        public IActionResult GetFabricsTypeList([FromBody]SearchGeneralFilterViewModel data)
        {
            ...
            var itemList = DbContext.FabricsTypes.Where(x =>
           ...
           .Where(z => z.IsLatest && !(z.IsDeleted) && z.IsActive).OrderBy(x => x.Name);
            ...
            // pass in the Model
            var result = new FabricsTypeListViewModel();
            result.Data = new List<FabricsTypeViewModel>();
            result.Paging = new Pagination();
            if (data.Paging.PageNumber == 0)
            {
                // if pagenumuber ==0 , get All
                result.Data = itemList.Select(x => TinyMapper.Map<FabricsTypeViewModel>(x)).ToList();
            ...
            result.Data = list.Select(x => TinyMapper.Map<FabricsTypeViewModel>(x)).ToList();
        ...
        // GET by Id
        [HttpGet("GetFabricsTypeById/{id}")]
        public IActionResult GetFabricsTypeById(Guid id)
        {
            Guid companyIdFromSearchData = Guid.Empty;
            Guid companyId = CompanyIdHelper.GetCompanyId(companyIdFromSearchData);
            var item = DbContext.FabricsTypes.Where(x => (companyId == Guid.Empty) || (x.CompanyId == companyId)).FirstOrDefault(i => i.Id == id);
            if (item != null) return new JsonResult(TinyMapper.Map<FabricsTypeViewModel>(item), DefaultJsonSettings);
            return NotFound(new { Error = $"ID {id} has not been found" });
        }


        // GET api/values/slug
        [HttpGet("GetFabricsTypeBySlug/{slug}")]
        public IActionResult GetFabricsTypeBySlug(string slug)
        {
            Guid companyIdFromSearchData = Guid.Empty;
            Guid companyId = CompanyIdHelper.GetCompanyId(companyIdFromSearchData);
            var item = DbContext.FabricsTypes.Where(x => (companyId == Guid.Empty) || (x.CompanyId == companyId)).Where(x=>x.IsLatest && !x.IsDeleted).FirstOrDefault(i => i.Slug == slug);
            if (item != null) return new JsonResult(TinyMapper.Map<FabricsTypeViewModel>(item), DefaultJsonSettings);
            return NotFound(new { Error = $"Slug {slug} has not been found" });
        }

        // Get item by code
        [HttpGet("GetFabricsTypeByCode/{code}")]
        public IActionResult GetFabricsTypeByCode(string code)
        {
        ...
        [HttpGet("GetFabricsTypeListByCode/{code}")]
        public IActionResult GetFabricsTypeListByCode(string code)
        {
            if (!string.IsNullOrEmpty(code))
            {
                Guid companyIdFromSearchData = Guid.Empty;
        ...
        // POST api/values
        [HttpPost("AddFabricsType")]
        public IActionResult Add([FromBody]FabricsTypeViewModel data)
        {
            if (data == null)
                return NotFound(new { error = "Invalid Data" });
            // return a HTTP Status 404 (Not Found) if we couldn't find a suitable item.
            ...
            string code = ch.GetTypeCode(EnumHelper.LibType.LibFabricsType.ToString());

            // Check if slug is duplicate or not
            string slug = SlugHelper.GetSlug(data.CompanyId, DbContext, data.Name, EnumHelper.LibType.LibFabricsType.ToString());
            var fabricstype = new LibFabricsType()
            {
                Id = Guid.NewGuid(),
            ...
            DbContext.FabricsTypes.Add(fabricstype);
        ...
        // PUT api/values/5
        [HttpPut("UpdateFabricsType")]
        [Authorize]
        public IActionResult Update([FromBody]FabricsTypeViewModel data)
        {
            if (data != null)
            {
                var fabricstype = DbContext.FabricsTypes.FirstOrDefault(i => i.Id.Equals(data.Id));
                ...
                if (fabricstype != null)
                {
                    ...
                    var fabtype = new LibFabricsType()
                    {
                        Id = Guid.NewGuid(),
                    ...
                    DbContext.FabricsTypes.Add(fabtype);
                    ...
                    DbContext.FabricsTypes.Update(fabricstype);
        ...
        // DELETE api/values/5
        [HttpDelete("DeleteFabricsType/{id}")]
        [Authorize]
        public IActionResult Delete(Guid id)
        {
            var item = DbContext.FabricsTypes.FirstOrDefault(i => i.Id.Equals(id));
            if (item != null)
            {
                item.IsDeleted = true;
                item.IsLatest = false;  
                DbContext.FabricsTypes.Update(item);
            }
            DbContext.SaveChanges();
        ...
        private List<FabricsTypeViewModel> ToItemViewModelList(IEnumerable<LibFabricsType> accessoryDesc)
        {
            var result = Enumerable.ToList(accessoryDesc.Select(TinyMapper.Map<FabricsTypeViewModel>));
            return result;
        }
