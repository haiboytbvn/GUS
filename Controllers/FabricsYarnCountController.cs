...
namespace GUSLibrary.Controllers
{
    ...
    public class FabricsYarnCountController : Controller
    {
        ...
        #region Constructor
        public FabricsYarnCountController(GUSLibraryDbContext context)
        {
            DbContext = context;
        }
        ...
        // GET: api/values
        [HttpPost("GetFabricsYarnCountList")]
        public IActionResult GetFabricsYarnCountList([FromBody]SearchGeneralFilterViewModel data)
        {
            ...
            var itemList = DbContext.FabricsYarnCounts.Where(x =>
           ...
           .Where(z => z.IsLatest && !(z.IsDeleted) && z.IsActive).OrderBy(x => x.Name);
            ...
            // pass in the Model
            var result = new FabricsYarnCountListViewModel();
            result.Data = new List<FabricsYarnCountViewModel>();
            result.Paging = new Pagination();
            if (data.Paging.PageNumber == 0)
            {
                // if pagenumuber ==0 , get All
                result.Data = itemList.Select(x => TinyMapper.Map<FabricsYarnCountViewModel>(x)).ToList();
            ...
            result.Data = list.Select(x => TinyMapper.Map<FabricsYarnCountViewModel>(x)).ToList();
        ...
        // GET by Id
        [HttpGet("GetFabricsYarnCountById/{id}")]
        public IActionResult GetFabricsYarnCountById(Guid id)
        {
            Guid companyIdFromSearchData = Guid.Empty;
            Guid companyId = CompanyIdHelper.GetCompanyId(companyIdFromSearchData);
            var item = DbContext.FabricsYarnCounts.Where(x => (companyId == Guid.Empty) || (x.CompanyId == companyId)).FirstOrDefault(i => i.Id == id);
            if (item != null) return new JsonResult(TinyMapper.Map<FabricsYarnCountViewModel>(item), DefaultJsonSettings);
            return NotFound(new { Error = $"ID {id} has not been found" });
        }


        // GET api/values/slug
        [HttpGet("GetFabricsYarnCountBySlug/{slug}")]
        public IActionResult GetFabricsYarnCountBySlug(string slug)
        {
            Guid companyIdFromSearchData = Guid.Empty;
            Guid companyId = CompanyIdHelper.GetCompanyId(companyIdFromSearchData);
            var item = DbContext.FabricsYarnCounts.Where(x => (companyId == Guid.Empty) || (x.CompanyId == companyId)).Where(x => x.IsLatest && !x.IsDeleted).FirstOrDefault(i => i.Slug == slug);
            if (item != null) return new JsonResult(TinyMapper.Map<FabricsYarnCountViewModel>(item), DefaultJsonSettings);
            return NotFound(new { Error = $"Slug {slug} has not been found" });
        }

        // Get item by code
        [HttpGet("GetFabricsYarnCountByCode/{code}")]
        public IActionResult GetFabricsYarnCountByCode(string code)
        {
        ...
        [HttpGet("GetFabricsYarnCountListByCode/{code}")]
        public IActionResult GetFabricsYarnCountListByCode(string code)
        {
            if (!string.IsNullOrEmpty(code))
            {
                Guid companyIdFromSearchData = Guid.Empty;
        ...
        // POST api/values
        [HttpPost("AddFabricsYarnCount")]
        public IActionResult Add([FromBody]FabricsYarnCountViewModel data)
        {
            if (data == null)
                return NotFound(new { error = "Invalid Data" });
            // return a HTTP Status 404 (Not Found) if we couldn't find a suitable item.
            ...
            string code = ch.GetTypeCode(EnumHelper.LibType.LibFabricsYarnCount.ToString());

            // Check if slug is duplicate or not
            string slug = SlugHelper.GetSlug(data.CompanyId, DbContext, data.Name, EnumHelper.LibType.LibFabricsYarnCount.ToString());
            var fabyarncount = new LibFabricsYarnCount()
            {
                Id = Guid.NewGuid(),
            ...
            DbContext.FabricsYarnCounts.Add(fabyarncount);

            // persist the changes into the Database.
            DbContext.SaveChanges();

            // set new id to data object
            data.Id = fabyarncount.Id;
        ...
        // PUT api/values/5
        [HttpPut("UpdateFabricsYarnCount")]
        [Authorize]
        public IActionResult Update([FromBody]FabricsYarnCountViewModel data)
        {
            if (data != null)
            {
                var fabyarncount = DbContext.FabricsYarnCounts.FirstOrDefault(i => i.Id.Equals(data.Id));

                if (fabyarncount != null)
                {
                    // Get company Id base on the user type
                    Guid companyIdFromSearchData = Guid.Empty;
                    Guid companyId = CompanyIdHelper.GetCompanyId(companyIdFromSearchData);
                    ...
                    var yarncount = new LibFabricsYarnCount()
                    {
                        Id = Guid.NewGuid(),
                        DateCreated = fabyarncount.DateCreated,
                        DateModified = fabyarncount.DateModified,
                        IsActive = fabyarncount.IsActive,
                        CompanyId = fabyarncount.CompanyId,
                        Name = fabyarncount.Name,
                        Code = fabyarncount.Code,
                        Version = fabyarncount.Version,
                        Slug = fabyarncount.Slug,
                        BuyerCode = fabyarncount.BuyerCode,
                        IsLatest = false,
                        IsDeleted = fabyarncount.IsDeleted
                    };
                    DbContext.FabricsYarnCounts.Add(yarncount);


                    // Update the old record data.
                    fabyarncount.DateModified = DateTime.Now;
                    fabyarncount.IsActive = data.IsActive;
                    fabyarncount.CompanyId = companyId;
                    fabyarncount.Name = data.Name;
                    fabyarncount.Version = fabyarncount.Version + 1;
                    fabyarncount.Slug = slug;
                    fabyarncount.BuyerCode = data.BuyerCode;
                    fabyarncount.IsLatest = true;
                    fabyarncount.IsDeleted = false;
                    DbContext.FabricsYarnCounts.Update(fabyarncount);
        ...
        // DELETE api/values/5
        [HttpDelete("DeleteFabricsYarnCount/{id}")]
        [Authorize]
        public IActionResult Delete(Guid id)
        {
            var item = DbContext.FabricsYarnCounts.FirstOrDefault(i => i.Id.Equals(id));
            if (item != null)
            {
                item.IsDeleted = true;
                item.IsLatest = false;
                DbContext.FabricsYarnCounts.Update(item);
            }
            DbContext.SaveChanges();
        ...
        private List<FabricsYarnCountViewModel> ToItemViewModelList(IEnumerable<LibFabricsYarnCount> accessoryDesc)
        {
            var result = Enumerable.ToList(accessoryDesc.Select(TinyMapper.Map<FabricsYarnCountViewModel>));
            return result;
        }
